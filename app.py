...
def index():
    ...
    if user:
        cur.execute("""
            SELECT OwnerID FROM internal_owners
            WHERE OwnerEmail = ?
        """, (user['Email'],))
...
def view_rma(rma_id):
    ...
    cur.execute("""
        SELECT ro.*, o.OwnerName, o.OwnerEmail
        FROM rma_owners ro
        JOIN internal_owners o ON ro.OwnerID = o.OwnerID
        WHERE ro.RMAID = ?
        ORDER BY ro.IsPrimary DESC, o.OwnerName
    """, (rma_id,))
...
def new_rma():
    ...
    if request.method == "POST":
        ...
        if owner_id:
            cur.execute("""
                INSERT INTO rma_owners (RMAID, OwnerID, IsPrimary, AssignedOn, AssignedBy)
                VALUES (?, ?, 1, ?, ?)
...
def change_primary_owner(rma_id):
    ...
    # Update legacy InternalOwnerID field for compatibility
    cur.execute("""
        UPDATE rmas
        SET InternalOwnerID = ?
        WHERE RMAID = ?
