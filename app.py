...
def login():
    if request.method == "POST":
        ...
        cur.execute(
            ...
            (datetime.now().isoformat(timespec="seconds"), user["UserID"])
        )
        conn.commit()
        conn.close()
...
def register():
    ...
    if request.method == "POST":
        ...
        is_owner = 1 if request.form.get("is_owner") else 0  # Get role from form
...
def index():
    ...
    # Recent RMAs
    cur.execute("""
        SELECT r.*, c.CustomerName, o.FullName, u.FullName as CreatedByName
        FROM rmas r
        JOIN customers c ON r.CustomerID = c.CustomerID
        LEFT JOIN users o ON r.AssignedToUserID = o.UserID
    ...
    # Check if current user is an owner and get their RMA counts
    user = get_current_user()
    is_owner = False
    my_rmas_count = 0
    urgent_rmas_count = 0
...
""", (user['Email'],))
        ...
        if owner_row:
            is_owner = True
            owner_id = owner_row['UserID']
            
            # Get count of assigned RMAs
            cur.execute("""
            ...
            """, (owner_id,))
            my_rmas_count = cur.fetchone()[0]
            
            # Get count of urgent RMAs (>14 days)
            cur.execute("""
                SELECT COUNT(DISTINCT r.RMAID)
                FROM rmas r
                JOIN rma_owners ro ON r.RMAID = ro.RMAID
                WHERE ro.UserID = ?
                  AND r.Status NOT IN ('Closed', 'Rejected')
                  AND DATE(r.DateOpened) <= DATE('now', '-14 days')
            """, (owner_id,))
            urgent_rmas_count = cur.fetchone()[0]
    ...
    return render_template(
        ...
        is_owner=is_owner,
        my_rmas_count=my_rmas_count,
        urgent_rmas_count=urgent_rmas_count
    )
...
def list_rmas():
    ...
    credit_approved = request.args.get("credit_approved")
    ...
    query = """
        ...
        LEFT JOIN users o
            ON r.AssignedToUserID = o.UserID
        LEFT JOIN users u
    ...
    # Credit approved filter
    if credit_approved:
        query += " AND r.CreditApproved = ?"
...
def view_rma(rma_id):
    ...
    cur.execute("""
        SELECT r.*, c.CustomerName, o.FullName, o.Email, u.FullName as CreatedByName
        FROM rmas r
        JOIN customers c ON r.CustomerID = c.CustomerID
        LEFT JOIN users o ON r.AssignedToUserID = o.UserID
        LEFT JOIN users u ON r.CreatedByUserID = u.UserID
    ...
    cur.execute("""
        SELECT ro.*, o.FullName, o.Email
        FROM rma_owners ro
        JOIN users o ON ro.UserID = o.UserID
        WHERE ro.RMAID = ?
        ORDER BY ro.IsPrimary DESC, o.FullName
    """, (rma_id,))
    ...
    if request.method == "POST":
        ...
        cur.execute("""
            ...
            (CustomerID, CreatedByUserID, DateOpened, Status, ReturnType, 
             AssignedToUserID, CustomerComplaintDesc, InternalNotes, NotesLastModified, NotesModifiedBy)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
        ...
        # Add owner to rma_owners table if specified
        if owner_id:
            cur.execute("""
                INSERT INTO rma_owners (RMAID, UserID, IsPrimary, AssignedOn, AssignedBy)
                VALUES (?, ?, 1, ?, ?)
            """, (rma_id, owner_id, date_opened, session['user_id']))
        ...
        if owner_id:
            cur.execute("SELECT OwnerName, OwnerEmail FROM internal_owners WHERE UserID = ?", (owner_id,))
            owner = cur.fetchone()
            if owner:
                ...
                send_rma_notification(
                    owner['Email'], owner['FullName'], rma_id, rma_code,
...
def list_owners():
    ...
    cur.execute("""
        ...
        LEFT JOIN rmas r ON o.UserID = r.AssignedToUserID
        GROUP BY o.UserID
        ORDER BY o.FullName
    """)
    owners = cur.fetchall()
...
def edit_owner(owner_id):
    ...
    if request.method == "POST":
        ...
        cur.execute("UPDATE internal_owners SET OwnerName = ?, OwnerEmail = ? WHERE UserID = ?",
                   (name, email, owner_id))
        conn.commit()
        conn.close()
    ...
    cur.execute("SELECT * FROM internal_owners WHERE UserID = ?", (owner_id,))
    owner = cur.fetchone()
    conn.close()
...
def delete_owner(owner_id):
    ...
    cur.execute("SELECT COUNT(*) FROM rmas WHERE AssignedToUserID = ?", (owner_id,))
    if cur.fetchone()[0] > 0:
        conn.close()
        flash('Cannot delete owner with existing RMAs.', 'error')
        return redirect(url_for("list_owners"))
    
    cur.execute("DELETE FROM internal_owners WHERE UserID = ?", (owner_id,))
    conn.commit()
    conn.close()
...
# ============ MULTIPLE OWNERS & ASSIGNMENTS ============

@app.route("/rmas/<int:rma_id>/owners/add", methods=["POST"])
@login_required
def add_rma_owner(rma_id):
    """Add an additional owner to an RMA"""
    owner_id = request.form.get("owner_id")
    
    if not owner_id:
        flash('Please select an owner.', 'error')
        return redirect(url_for("view_rma", rma_id=rma_id))
    ...
    cur.execute("""
        SELECT COUNT(*) FROM rma_owners
        WHERE RMAID = ? AND UserID = ?
    """, (rma_id, owner_id))
    
    if cur.fetchone()[0] > 0:
        conn.close()
        flash('This owner is already assigned to this RMA.', 'warning')
        return redirect(url_for("view_rma", rma_id=rma_id))
    ...
    cur.execute("""
        INSERT INTO rma_owners (RMAID, UserID, IsPrimary, AssignedOn, AssignedBy)
        VALUES (?, ?, 0, ?, ?)
    """, (rma_id, owner_id, now, session.get('user_id')))
...
def remove_rma_owner(rma_id, owner_id):
    ...
    cur.execute("""
        SELECT IsPrimary FROM rma_owners
        WHERE RMAID = ? AND UserID = ?
    """, (rma_id, owner_id))
    ...
    if not row:
        conn.close()
        flash('Owner not found on this RMA.', 'error')
        return redirect(url_for("view_rma", rma_id=rma_id))
    
    if row[0] == 1:  # IsPrimary
        conn.close()
        flash('Cannot remove primary owner. Assign a new primary owner first.', 'error')
        return redirect(url_for("view_rma", rma_id=rma_id))
...
@app.route("/rmas/<int:rma_id>/owners/primary", methods=["POST"])
@login_required
def change_primary_owner(rma_id):
    """Change the primary owner of an RMA"""
    new_primary_id = request.form.get("owner_id")
    ...
    # Check if new primary is already an owner
    cur.execute("""
        SELECT COUNT(*) FROM rma_owners
        WHERE RMAID = ? AND UserID = ?
    """, (rma_id, new_primary_id))
    
    is_already_owner = cur.fetchone()[0] > 0
    
    # Set all current owners to non-primary
    cur.execute("""
        UPDATE rma_owners
        SET IsPrimary = 0
        WHERE RMAID = ?
    ...
    if is_already_owner:
        # Update existing owner to primary
        cur.execute("""
            UPDATE rma_owners
            SET IsPrimary = 1
            WHERE RMAID = ? AND UserID = ?
        ...
        cur.execute("""
            INSERT INTO rma_owners (RMAID, UserID, IsPrimary, AssignedOn, AssignedBy)
            VALUES (?, ?, 1, ?, ?)
        """, (rma_id, new_primary_id, now, session.get('user_id')))
    
    # Update legacy AssignedToUserID field for compatibility
    cur.execute("""
        UPDATE rmas
        SET AssignedToUserID = ?
        WHERE RMAID = ?
    """, (new_primary_id, rma_id))
    
    conn.commit()
    conn.close()
    
    flash('Primary owner updated successfully.', 'success')
    return redirect(url_for("view_rma", rma_id=rma_id))
...
@login_required
def dashboard():
    """Owner's personalized dashboard showing their assigned RMAs"""
    user = get_current_user()
    
    conn = get_db()
    cur = conn.cursor()
    
    # Get user's owner record (if they are an owner)
    cur.execute("""
        SELECT UserID FROM users
        WHERE IsOwner = 1 AND Email = ?
    """, (user['Email'],))
    ...
    # Get all RMAs assigned to this owner
    cur.execute("""
        SELECT DISTINCT r.*, c.CustomerName,
               ro.IsPrimary
        FROM rmas r
        JOIN rma_owners ro ON r.RMAID = ro.RMAID
        JOIN customers c ON r.CustomerID = c.CustomerID
    ...
    # Calculate stats
    total_assigned = len(my_rmas)
    urgent_count = 0  # >14 days
    warning_count = 0  # 7-14 days
    normal_count = 0  # <7 days
...
def notification_preferences():
    ...
    if owner_row:
        ...
        cur.execute("""
            SELECT * FROM owner_notification_preferences
            WHERE UserID = ?
        """, (owner_id,))
        ...
        if not prefs:
            cur.execute("""
                ...
                (UserID, EmailEnabled, Frequency, RMAAge)
                VALUES (?, 1, 'daily', 3)
            """, (owner_id,))
            conn.commit()
...
@app.route("/profile/notifications/save", methods=["POST"])
@login_required
def save_notification_preferences():
    ...
    if not owner_row:
        conn.close()
        flash('You must be an internal owner to configure notifications.', 'error')
        return redirect(url_for('profile'))
    ...
    rma_age = int(request.form.get('rma_age', 3))
    
    # Validate frequency
    valid_frequencies = ['daily', 'every_3_days', 'weekly']
    if frequency not in valid_frequencies:
        frequency = 'daily'
    
    # Validate age (allow 1-365 days)
    if rma_age < 1:
        rma_age = 1
    elif rma_age > 365:
        rma_age = 365
    ...
    cur.execute("""
        ...
        SET EmailEnabled = ?,
            ...
            RMAAge = ?
        WHERE UserID = ?
    """, (email_enabled, frequency, rma_age, owner_id))
...
def admin_users():
    ...
    cur.execute("""
        SELECT UserID, Username, FullName, Email, Role, CreatedOn, LastLogin
        FROM users
...
def delete_user(user_id):
    ...
    # Check if user has created RMAs
    cur.execute("SELECT COUNT(*) FROM rmas WHERE CreatedByUserID = ?", (user_id,))
    rma_count = cur.fetchone()[0]
    
    if rma_count > 0:
        conn.close()
        flash(f'Cannot delete user who has created {rma_count} RMAs. Reassign RMAs first.', 'error')
        return redirect(url_for('admin_users'))
...
def edit_user(user_id):
    ...
    if request.method == "POST":
        ...
        if not all([full_name, email]):
            ...
            return render_template("edit_user.html", user=user, is_self=(user_id == current_user['UserID']))
